...
export default function ProductDetailPage() {
  ...
  const [product, setProduct] = useState<CatalogProductItem | null>(null);
  ...
  if (!product) {
    return (
      <div style={{ padding: '2rem 1rem', textAlign: 'center' }}>
        ...
        <Link href={`/store/${slug}`}>Kembali ke katalog</Link>
      </div>
    );
  }
  ...
  function validateQty(qty: number): string | null {
    if (hasVariants && !selectedVariant) {
      return 'Pilih varian produk terlebih dahulu.';
    }
    if (qty < moq) {
      return `Min. order ${moq} ${unitSymbol}.`;
    }
    if (qty % orderStep !== 0) {
      return `Kelipatan order: ${orderStep} ${unitSymbol}.`;
    }
    return null;
  }
  ...
  function handleVariantChange(variantId: string) {
    ...
    if (variant) {
      setQuantity(Math.max(variant.moq, 1));
    }
  }
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      color: disabled ? colors.light.text.secondary : colors.light.text.primary,
      ...
      {toast ? (
        <div
          style={{
            ...
            left: '50%',
            transform: 'translateX(-50%)',
            background: colors.light.text.primary,
